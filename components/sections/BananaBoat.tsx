    ...
    <section id="banana" ref={ref as React.RefObject<HTMLElement>} className="bg-ice pt-20 pb-5 px-5 relative">
      <div className="max-w-[1140px] mx-auto">
        <div
          ...
          className="rounded-[36px] overflow-hidden relative bg-gradient-to-br from-gold via-sun to-coral flex flex-wrap min-h-[460px] shadow-[0_20px_60px_rgba(255,140,56,0.13)] transition-all duration-[1100ms] ease-expo"
        ...
        >
          ...
          <div className="flex-[1_1_380px] p-8 md:p-12 lg:p-14 flex flex-col justify-center relative z-[2]">
            <h2
              className="font-display text-white leading-[0.9] mb-[18px]"
              style={{ fontSize: "clamp(40px, 6vw, 68px)", textShadow: "0 4px 20px rgba(0,0,0,0.15)" }}
            >
              {dict.title1}<br />{dict.title2}
            </h2>
            <p
              className="font-body text-white/[0.92] leading-[1.65] mb-8 max-w-[420px]"
            ...
            <div className="flex gap-3 flex-wrap">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="font-body py-[15px] px-9 rounded-full border-none bg-white text-coral font-extrabold text-base cursor-pointer shadow-[0_8px_24px_rgba(0,0,0,0.12)] transition-all duration-400 ease-expo hover:scale-105 no-underline inline-block"
              >
                {dict.cta}
              </a>
              <Link
                href={`/${locale}/actividades/banana-boat`}
                className="font-body py-[15px] px-7 rounded-full border-2 border-white/50 bg-transparent text-white font-semibold text-[15px] transition-all duration-300 hover:bg-white/10 no-underline inline-block"
              >
                {dict.price}
              </Link>
            </div>
