    ...
    <section ref={ref as React.RefObject<HTMLElement>} className="bg-gradient-to-b from-deep to-midnight py-14 sm:py-20 md:py-28 px-4 sm:px-5 relative overflow-hidden">
      ...
      <div className="max-w-[1140px] mx-auto relative z-[2] text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-[18px]">
          {dict.items.map((r, i) => (
            <div key={i} className="bg-gradient-to-br from-turq/[0.03] to-ocean/[0.03] backdrop-blur-[16px] border border-turq/[0.08] rounded-[20px] sm:rounded-[28px] p-5 sm:p-6 md:p-8 text-left"
              ...
              <p className="font-body text-[13px] sm:text-[15px] text-white/[0.78] leading-[1.5] sm:leading-[1.6] mb-4 sm:mb-5">&ldquo;{r.text}&rdquo;</p>
              <div className="flex items-center gap-2.5 sm:gap-3">
                <div className="w-8 h-8 sm:w-[38px] sm:h-[38px] rounded-full bg-gradient-to-br from-turq to-ocean flex items-center justify-center font-body font-extrabold text-white text-[13px] sm:text-[15px]">{r.name[0]}</div>
                <div>
