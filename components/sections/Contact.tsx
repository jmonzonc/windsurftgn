...
export default function Contact({ dict }: { dict: ContactDict }) {
  ...
  const hrefs = [SITE.phoneHref, SITE.mapsUrl, "https://instagram.com/windsurftarragona", SITE.whatsapp];
    ...
    <section id="contacto" ref={ref as React.RefObject<HTMLElement>} className="bg-gradient-to-b from-ice to-white py-28 px-5 relative">
      <div className="max-w-[960px] mx-auto text-center">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4"
          ...
          {items.map((item, i) => <ContactCard key={i} icon={ICONS[i]} href={hrefs[i]} {...item} />)}
        </div>
        <div className="mt-12 rounded-[28px] overflow-hidden h-[280px] relative bg-gradient-to-br from-ocean/[0.08] to-turq/[0.06] border border-ocean/[0.06]"
          ...
          <iframe src={SITE.mapsEmbed}
            width="100%" height="100%" className="border-0 saturate-[0.7] brightness-[1.05]" allowFullScreen loading="lazy" title="Ubicación Windsurf Tarragona" />
          <div className="absolute bottom-5 left-5 right-5 bg-white/[0.92] backdrop-blur-[16px] rounded-2xl py-4 px-6 flex justify-between items-center shadow-[0_8px_24px_rgba(0,0,0,0.08)] flex-wrap gap-3">
            ...
            <a href={SITE.mapsUrl} target="_blank" rel="noreferrer" className="btn-primary py-2.5 px-[22px] text-[13px] no-underline">{dict.mapCta}</a>
          </div>
        </div>
      </div>
    </section>
...
function ContactCard({ icon, title, text, sub, href }: { icon: string; title: string; text: string; sub: string; href: string }) {
  const [h, setH] = useState(false);
  return (
    <a href={href} target={href.startsWith("tel:") ? undefined : "_blank"} rel="noreferrer" className="no-underline"
      onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}>
      <div className="bg-white rounded-3xl py-9 px-[22px] cursor-pointer transition-all duration-400 ease-expo h-full"
        style={{ boxShadow: h ? "0 12px 36px rgba(0,212,170,0.1)" : "0 2px 16px rgba(0,104,214,0.02)", border: h ? "2px solid rgba(0,212,170,0.2)" : "2px solid rgba(0,104,214,0.03)", transform: h ? "translateY(-5px)" : "translateY(0)" }}>
        <div className="text-4xl mb-3.5 transition-transform duration-400 ease-expo" style={{ transform: h ? "scale(1.15)" : "scale(1)" }}>{icon}</div>
