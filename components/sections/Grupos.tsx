...
export default function Grupos({ dict, locale }: { dict: GruposDict; locale: Locale }) {
  ...
  return (
    <section id="grupos" ref={ref as React.RefObject<HTMLElement>} className="bg-gradient-to-b from-sand via-white to-ice py-28 px-5 relative overflow-hidden">
      <div className="max-w-[960px] mx-auto text-center relative z-[1]">
        <div style={{ opacity: vis ? 1 : 0, transform: vis ? "translateY(0)" : "translateY(40px)", transition: "all 0.9s cubic-bezier(0.16,1,0.3,1)" }}>
          <h2 className="font-display text-midnight leading-none mb-14" style={{ fontSize: "clamp(34px, 5.5vw, 56px)" }}>
        ...
        <div className="mt-14" style={{ opacity: vis ? 1 : 0, transform: vis ? "translateY(0)" : "translateY(20px)", transition: "all 0.8s cubic-bezier(0.16,1,0.3,1) 0.6s" }}>
          <Link href={`/${locale}/grupos`} className="btn-primary py-[17px] px-12 text-[17px] font-extrabold no-underline inline-block shadow-[0_10px_36px_rgba(0,104,214,0.16)] hover:-translate-y-1 hover:scale-[1.04]">{dict.cta}</Link>
        </div>
      </div>
    </section>
  );
...
function GroupCard({ emoji, title, desc, vis, delay }: { emoji: string; title: string; desc: string; vis: boolean; delay: number }) {
  ...
  return (
    <div onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      className="bg-white rounded-[28px] px-6 pt-10 pb-9 cursor-pointer transition-all duration-500 ease-expo"
      ...
      <div className="text-[44px] mb-4 transition-transform duration-400 ease-expo" style={{ transform: h ? "scale(1.15) rotate(-5deg)" : "scale(1)" }}>{emoji}</div>
      <h3 className="font-display text-xl text-midnight mb-2">{title}</h3>
      <p className="font-body text-sm text-gray-400 leading-[1.5]">{desc}</p>
    </div>
  );
}
